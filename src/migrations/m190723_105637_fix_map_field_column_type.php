...
class m190723_105637_fix_map_field_column_type extends Migration
{
    ...
    public function safeUp()
    {
        ...
        foreach ($fields as $fieldUid => $field)
	        if ($field['type'] === MapField::class)
		        $updates['fields.' . $fieldUid . '.contentColumnType'] = 'text';

        foreach ($matrixBlockTypes as $blockTypeUid => $blockType)
        	foreach ($blockType['fields'] as $fieldUid => $field)
        		if ($field['type'] === MapField::class)
        			$updates['matrixBlockTypes.' . $blockTypeUid . '.fields.' . $fieldUid . '.contentColumnType'] = 'text';

        foreach ($superTableBlockTypes as $blockTypeUid => $blockType)
        	foreach ($blockType['fields'] as $fieldUid => $field)
        		if ($field['type'] === MapField::class)
        			$updates['superTableBlockTypes.' . $blockTypeUid . '.fields.' . $fieldUid . '.contentColumnType'] = 'text';
